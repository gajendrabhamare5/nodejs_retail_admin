...
function placeorder() {
    ...
    $.ajax({
        type: "POST",
        url: baseurl + "ajaxfiles/a_place_order.php",
        type: 'post',
        dataType: "json",
        data: fd,
        contentType: false,
        ...
        success: function (data) {
            if (data.status == "ok") {
                ...
                if (paymentmethod == "Online Payments" && ordertotal != 0 && redirect_online == 'yes') {
                    ...
                    window.location.href = baseurl + "order_info/" + orid;
                }
            }
...
function paymentprice(type) {
    ...
    if (type == "razorpay") {
        ...
        if (is_apply_cod == 1 && transaction_charge > 0) {

            $(".show_cod").show();
            console.log(transaction_charge, 'transaction_charge');
            console.log(cod_percent, 'cod_percent');
        }
    } else {
        var charge1 = (parseInt(cartvalue) * 7.40) / 100;
        var transaction_charge = charge1 + (charge1 * 18) / 100 + 0.30;
