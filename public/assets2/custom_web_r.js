...
$(window).on('scroll', function() {
    ...
    var hT = $('#content-product-area').offset().top,
        ...
        wH = $(window).height(),
        wS = $(this).scrollTop();
        if (wS > (hT + hH - wH) && isloading == true && total_products > 0) {
        console.log("RR");
        ...
        $.ajax({
            ...
            /* $('#pagination-limit-start').val(parseInt(pagination_limit_start) + 20); */
            if (data == null || data == "") {
                console.log("if");
                isloading = false;
                $('#errordiv').html("<span style='text-align: center;'>That's All friends!.</span><br><br>");
            }
            else
            {
                response = data;
                ...
                /* setTimeout(function() { */
                    ...
                    pagination_limit_start = parseInt(pagination_limit_start) + 20;
                    isloading = true;
              /*   }, 5000); */
            }
        });
...
window.addEventListener("popstate", function () {
    ...
    var new_fulllink = baseurl + "catalog_filter_r.php" + newlink + "start=" + page + "&loadmore=call&seo_id=" + seo_id + abc + order;
    $.ajax({
        url: new_fulllink,
        beforeSend: function () {
            isloading = false;
        ...
        if (data.trim() == null || data.trim() == "") {
            $('#errordiv').html("<span style='text-align: center;'>No more catalog found.</span><br><br>");
        } else {
            alert();
            $('#errordiv').html(data);
        }
...
function filterdata() {
    ...
    $.ajax({
        url: baseurl + "catalog_filter_r.php" + linkGet,
    }).done(function (data) {
        $('#filter_data').html(data);
        if (data == null || data == "") {
            $(".loader").hide();
...
function calculate_grandtotal() {
    ...
    var currency_cost = c_convert;
    var grand_total = parseInt(subtotal) + parseInt(shipping_charge11) + parseInt(transaction_charge) + parseInt(handling_chrg) - parseInt(use_point) - parseInt(scheme_discount);
